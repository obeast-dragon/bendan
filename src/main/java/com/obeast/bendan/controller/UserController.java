...
public class UserController {
    ...
    public R<PageObjects<UserEntity>> list(@RequestParam Map<String, Object> params) {
        if (params.size() == 0) {
            return R.error("params is null");
        }
        PageObjects<UserEntity> page = userService.queryPage(params);
    ...
    public R<List<UserEntity>> listAll() {
    ...
    public R<UserEntity> getOneById(@PathVariable("id") Long id){
        if (id < 0){
            return R.error("id is null");
        }
		UserEntity user = userService.queryById(id);
        return R.success(user, "user");
    }
    ...
    public R save(@Validated({AddGroup.class, DefaultGroup.class}) @RequestBody UserEntity userentity){
        if (userentity == null){
            return R.error("userentity must not be null");
        }
        boolean flag = userService.add(userentity);
        if (flag) {
    ...
    public R saveList(@Validated({AddGroup.class, DefaultGroup.class}) @RequestBody List<UserEntity> data){
        if (data.size() == 0) {
            return R.error("data must not be null");
        }
        boolean flag = userService.addList(data);
        if (flag) {
    ...
    public R update(@Validated({UpdateGroup.class, DefaultGroup.class}) @RequestBody UserEntity userentity){
        if (userentity == null){
            return R.error("userentity must not be null");
        }
        boolean flag = userService.replace(userentity);
        if (flag){
    ...
    public R updateList(@Validated({UpdateGroup.class, DefaultGroup.class}) @RequestBody List<UserEntity> data) {
        if (data.size() == 0) {
            return R.error("data must not be null");
        }
        boolean flag = userService.replaceList(data);
        if (flag){
    ...
    public R deleteList(@Validated({DeleteGroup.class, DefaultGroup.class}) @RequestParam("ids") List<Long> ids) {
        ...
        boolean removes = userService.deleteByIds(ids);
        if (removes){
    ...
    public void export(HttpServletResponse response,
        ...
        if (fileName == null) {
            ...
            List<UserExcel> data = userService.queryExcelByConditions();
            EasyExcelUtils<UserExcel> easyExcel = new EasyExcelUtils<>(UserExcel.class);
